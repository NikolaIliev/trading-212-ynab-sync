...
import { T212TransactionSchema, type T212Transaction } from './schemas/T212Transaction'
import z from 'zod'
import { delay } from './utils/delay'

const T212ApiKey = z.string().parse(process.env.T212_API_KEY)
const T212ImportStartDate = z.string().optional().parse(process.env.T212_IMPORT_START_DATE)
...
const ynabAccount = ynabBudget.accounts?.find(account => account.type === 'checking' && account.name === 'T212 (Cash)')
...
async function requestTrading212Report(): Promise<number> {
  const timeFrom = new Date(
    Math.max(T212ImportStartDate ? new Date(T212ImportStartDate).getTime() : 0,
      // 30 days ago
      new Date().getTime() - 1000 * 60 * 60 * 24 * 30)
  ).toISOString()

  console.log(`[T212] Generating report from ${timeFrom} until today...`)
  const res = await fetch("https://live.trading212.com/api/v0/history/exports", {
    method: "POST",
    body: JSON.stringify({
      dataIncluded: {
...
csv.parseString(csvStr, { headers: true }).on('data', rawTransaction => {
  if (!("Action" in rawTransaction)) {
    throw new Error(`Expected rawTransaction to have "Action" field: ${rawTransaction}`)
  }
  ...
  if (rawTransaction.Action === 'Card debit') {
    ...
    if (!transaction.success) {
      throw new Error(`Failed to parse Card debit transaction: ${rawTransaction} ${transaction}`)
    }
